...
```jsx react-live
function filter() {
  const defBusinessTypes = React.useRef(["eco-accommodations",
  "ethical experiences","Resposible Travel Curators","socially responsible eateries",
  ]);

  const countryNames = React.useRef([
    "Hong Kong","India","Indonesia","Kenya",
    "Laos","Malaysia","Philippines","United States Of America",
  ]);

  const country = React.useRef({
    All: {
      BusinessTypes: [
        "eco-accommodations","ethical experiences","Resposible Travel Curators",
        "socially responsible eateries","Ancient Sites"
      ]
    },
    "Hong Kong": {
    ...
    Indonesia: {
      BusinessTypes: ["eco-accommodations", "ethical experiences"]
    },
    Kenya: {
      BusinessTypes: ["eco-accommodations", "Resposible Travel Curators",]
    },
    Laos: {
      BusinessTypes: ["eco-accommodations", "socially responsible eateries", "Ancient Sites"]
    },
    Malaysia: {
      BusinessTypes: ["eco-accommodations", "Resposible Travel Curators", "Ancient Sites"]
    },
    Philippines: {
      BusinessTypes: ["eco-accommodations", "socially responsible eateries"]
    },
  ...
  const [businessTypesState, setBusinessTypesState] = React.useState(defBusinessTypes.current);
  const [priceState, setPriceState] = React.useState([
    "all prices","under $200","$200 - $500","over $500",
  ]);
  ...
  function selectCountry(place) {
    if (countryDropdownState[0] === "All") {
      ...
      for (let i = 0; i < selectArray.length; i++) {
        ...
        if (el.innerHTML === place) el.classList.add("bold");
      }
  ...
  function deleteCountrySelection() {
    ...
    setBusinessTypesState(["eco-accommodations","ethical experiences",
      "Resposible Travel Curators","socially responsible eateries",
    ]);
  }
  function selectBusinessType(business) {
    if (businessDropdownState[0] === "All") {
      ...
      for (let i = 0; i < selectArray.length; i++) {
        let el = selectArray[i].firstChild;
        if (el.innerHTML === business) el.classList.add("bold");
      }
    } else {
      if (businessDropdownState.includes(business)) {
        ...
        for (let i = 0; i < selectArray.length; i++) {
          let el = selectArray[i].firstChild;
          if (el.innerHTML === business) el.classList.remove("bold");
        }
        let array = [...businessDropdownState];
        array.splice(index, 1);
        if (array.length < 1) setBusinessDropdownState(["All"])
        else setBusinessDropdownState(array);
      } else {
        let array = [...businessDropdownState];
        array.push(business);
        setBusinessDropdownState(array);
  ...
  function selectPrice(price) {
    if (priceDropdownState[0] === "All") {
      ...
      for (let i = 0; i < selectArray.length; i++) {
        ...
        if (array.includes(el)) selectArray[i].firstChild.classList.add("bold");
      }
    } else {
      if (!priceDropdownState.includes(price)) {
        ...
        if (priceDropdownState.length === 1) {
          ...
          for (let i = 0; i < selectArray.length; i++) {
            let el = selectArray[i].firstChild;
            if (el.innerHTML === price) el.classList.remove("bold");
          }
        }
      }
    }
